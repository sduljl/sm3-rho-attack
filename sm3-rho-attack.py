...
def CF(digest, W, W1):
    ...
    for j in range(0, 64):
        ...
        E = P0(TT2)
    temp = int(
        int2bin(A, 32) + int2bin(B, 32) + int2bin(C, 32) + int2bin(D, 32) + int2bin(E, 32) + int2bin(F, 32) + int2bin(G,
                                                                                                                      32) + int2bin(
            H, 32), 2)
...
def rho_attack(n):
    m = random.randint(0, pow(2, 64))
    h_ = []
    for i in range(0, pow(2, 32)):
        h_.append(sm3(m)[:int(n / 4)])
        m = 2 * m + 1
        if sm3(m)[:int(n / 4)] in h_:
            print("Succeed")
            return
    print("Failed")
